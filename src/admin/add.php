...
$array_staff_forms = [
  'staff__name__kanji', 'staff__name__kana', 'staff__tel',  'staff__email', 'staff__detail'

];

$array_staff_message = [
  '担当者様氏名(漢字)', '担当者様氏名(フリガナ)', '電話番号', 'メールアドレス', '担当者紹介文'
];
...
// $service_length = count($_SESSION['support']);
// $agents_count_stmt = $db->prepare("SELECT COUNT(*) from agents");
// $agents_count_stmt->execute();
// $agents_count_data = $agents_count_stmt->fetchAll();
// $agents_count = $agents_count_data[0]['COUNT(*)'];
// // for ($j = 0; $j <= $service_length - 1; $j++) {
...
// echo $_SESSION['support'][$j];
// }
// echo  $_POST['service__detail'];
            ...
            <form action="" name="apply__form" class="apply__form" method="post">
              ...
              <dl class="apply__form__list">
                ...
                <div class="apply__form__item">
                  ...
                  //   $service_length=count($_POST['service__support']);
                  //   $agents_count_stmt = $db->prepare("SELECT COUNT(*) from agents");
                  // $agents_count_stmt->execute();
                  // $agents_count_data = $agents_count_stmt->fetchAll();
                  // $agents_count = $agents_count_data[0]['COUNT(*)'];
                  //   for($j=0;$j<=$service_length-1;$j++){
                  //     $_SESSION['service__support'][$j]=$_POST['service__support'][$j];
                  //   $_SESSION['service__support'][$j]=$_POST['service__support'][$j];
                  //   $agents_supports_connect = $db->exec('INSERT INTO agents_supports_connect SET
                  //   agent_id="' . $agents_count . '",
                  //   support_id="' . $_SESSION['service__support'][$j] . '"
                  //   ');
                  //   echo $_SESSION['service__support'][$j];} 
                  // echo $service_length;
                  // }
